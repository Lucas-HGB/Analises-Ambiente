...
def run_command(command):
    output = system("echo '%s' | sqlplus -s / as sysdba"%(command))
    return output


def print_menu():
    print "1  - Dados da instância"
    print "2  - Versão dos produtos instalados"
    print "3  - Configurações de NLS da Instância"
    print "4  - Informações sobre os controlfiles"
    print "5  - Informações sobre Redologs"
    print "6  - Tablespaces"
    print "7  - Parâmetros da Instância"
    print "8  - Hits de Memória do Oracle"
    print "9  - Advisors"
    print "10  - Data Dictionary gets"
    print "11 - Table Scan"
    print "12 - Database Misses"
    print "13 - Blocos Buffer Cache"
    print "14 - Memória Oracle"
    print "15 - Sair\n"
...
def init():
    ...
    while True:
        print_menu()
        extract_info = input() - 1
        system("clear")
        if extract_info != 15:
            run_command(scripts[extract_info])
        else:
            break
